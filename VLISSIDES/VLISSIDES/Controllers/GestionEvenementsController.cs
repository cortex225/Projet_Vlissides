...
public class GestionEvenementsController : Controller
{
    ...
    public IActionResult Index()
    {
        ...
        var evenementsSupprime = _context.Evenements.Include(e => e.Reservations)
    ...
    {
        var reservations = _context.Reservations.Include(r => r.Evenement).Include(r => r.Membre)
            .Where(r => r.EnDemandeAnnuler == true).Select(r => new GestionEvenementsReservationVM
            {
                Id = r.Id,
    ...
    private string BuildConfirmationEmailMembreConfirmedBody(string username, Reservation reservation, string logoUrl)
    {
        ...
        var myURL = _httpContextAccessor.HttpContext.Request.Host
            .Value; //_httpContextAccessor.Request.Host.Value;            string BASE_URL_RAZOR = Url.Content("~"); 
        ...
        body.Append(
            $"<h2 style='color: #444;'>Votre demande d'annulation de la commande {reservation.Id} a été accepté!</h2>");
        ...
        body.Append(
            $"<p style='color: #555; font-size: 16px;'><strong>Remboursement Total:</strong> {reservation.prixAchat}</p>");
        body.Append(
    ...
    private string BuildConfirmationEmailMembreCancelledBody(string username, Reservation reservation, string logoUrl)
    {
        ...
        body.Append($"<h2 style='color: #333; text-align: center; margin-top: 0;'>Cher(e) {username},</h2>");
        body.Append(
            $"<h2 style='color: #444;'>Votre demande d'annulation de la commande {reservation.Id} a été refusé.</h2>");
