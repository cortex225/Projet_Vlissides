...
public class EvenementsController : Controller
{
    ...
    {
        var vm = new EvenementsIndexVM();

        if (User.Identity.IsAuthenticated)
        {
            //Pour trouver les reservations du membres
            var userId = _userManager.GetUserId(HttpContext.User);
            var reservations = _context.Reservations.Include(r => r.Evenement).Where(r => r.MembreId == userId)
                .ToList();
            //Transformer les evenements réservé en vm
            var mesEvenements = new List<EvenementsVM>();
            foreach (var reservation in reservations)
                mesEvenements.Add(new EvenementsVM
                {
                    ...
                    NbPlaces = reservation.Evenement.Reservations == null
                        ? reservation.Evenement.NbPlaces + "/" + reservation.Evenement.NbPlaces
                        : reservation.Evenement.NbPlaces - reservation.Evenement.Reservations.Count + "/" +
                          reservation.Evenement.NbPlaces,
                    NbPlacesMembre = reservation.Evenement.Reservations == null
                        ? reservation.Evenement.NbPlacesMembre + "/" + reservation.Evenement.NbPlacesMembre
                        : reservation.Evenement.NbPlacesMembre - reservation.Evenement.Reservations.Count() + "/" +
                          reservation.Evenement.NbPlacesMembre,
            ...
            vm.Evenements = _context.Evenements.Include(e => e.Reservations).ThenInclude(r => r.Membre).ToList()
                ...
                .Select(e => new EvenementsVM
                {
                    ...
                    Image = e.Image,
                    Lieu = e.Lieu,
                    NbPlaces = e.Reservations == null
                        ? e.NbPlaces + "/" + e.NbPlaces
                        : e.NbPlaces - e.Reservations.Count + "/" + e.NbPlaces,
                    NbPlacesMembre = e.Reservations == null
                        ? e.NbPlacesMembre + "/" + e.NbPlacesMembre
                        : e.NbPlacesMembre - e.Reservations.Count() + "/" + e.NbPlacesMembre,
                    Prix = e.Prix,
    ...
    {
        var userId = _userManager.GetUserId(HttpContext.User);
        var reservation = _context.Reservations.Include(r => r.Evenement)
            .FirstOrDefault(r => r.EvenementId == id && r.MembreId == userId);
    ...
    private string BuildEmailAdminAnnulerBody(string username, Reservation reservation, string logoUrl)
    {
        ...
        body.Append("<a href=" + "https://sqlinfocg.cegepgranby.qc.ca/2147186" +
                    ">Aller à la page de gestion de réservations</a>");
        body.Append("</div>");
    ...
    public ActionResult CreateCheckoutSessionEvenement(string id)
    {
        ...
        var encodedImgEvenementUrl =
            Uri.EscapeUriString(
                imgEvenementUrl); // Encodez l'URL de l'image du livre pour qu'elle soit utilisable dans Stripe
