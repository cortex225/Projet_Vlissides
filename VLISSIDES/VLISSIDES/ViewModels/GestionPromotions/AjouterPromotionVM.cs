    ...
    public class AjouterPromotionVM
    {
            ...
            List<Categorie> categories = null, List<MaisonEdition> maisonEditions = null, List<string> typePromotions = null)
        {
            if (promotion == null)
            {
                ...
                ImageUrl = "";
                Auteur = "";
                Auteurs = auteurs.Select(a => a.NomAuteur).ToList() ?? new List<string>();
                Categorie = "";
                Categories = categories.Select(c => c.Nom).ToList() ?? new List<string>();
                MaisonEdition = "";
                MaisonEditions = maisonEditions.Select(me => me.Nom).ToList() ?? new List<string>();
                TypePromotion = "";
